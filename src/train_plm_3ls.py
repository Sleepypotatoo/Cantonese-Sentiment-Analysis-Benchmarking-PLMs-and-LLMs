...
TRAIN_PATH = "./data/openrice_3cls/train.tsv"      # 训练集路径（TSV格式）
TEST_PATH = "./data/openrice_3cls/test.tsv"        # 测试集路径（TSV格式）
MAX_LEN = 128
OUTPUT_DIR = "./saved_models/openrice_3cls_model"  # 保存目录

def load_data(file_path):
    """读取TSV文件，包含label和text_a两列"""
    df = pd.read_csv(file_path, sep='\t', on_bad_lines='skip')
    # 只保留需要的列，并删除空值
    df = df[['label', 'text_a']].dropna()
    # 将文本列重命名为 'text' 方便后续处理
    df = df.rename(columns={'text_a': 'text'})
    
    # 将标签映射为数字
    unique_labels = df['label'].unique()
    print(f"发现标签: {unique_labels}")
    # 建立标签到ID的映射（按字母排序保证一致性）
...
def train():
    # 加载训练集和测试集（已按8:2划分好）
    train_df, label_to_id, id_to_label = load_data(TRAIN_PATH)
    ...
    # 计算类别权重
    class_weights = compute_class_weight('balanced', classes=np.arange(num_labels), y=train_df['label_id'])
    ...
    train_dataset = CantoneseDataset(train_df['text'].tolist(), train_df['label_id'].tolist(), tokenizer, MAX_LEN)
    test_dataset = CantoneseDataset(test_df['text'].tolist(), test_df['label_id'].tolist(), tokenizer, MAX_LEN)
    ...
    results_df = pd.DataFrame({
        ...
        'true_label': [id_to_label[y] for y in y_true],
        'pred_label': [id_to_label[y] for y in y_pred],
        'true_id': y_true,
        'pred_id': y_pred
    })
